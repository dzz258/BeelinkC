...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.alpha = self.configure.indicatorAlpha;
...
- (void)setupTitleButtons {
    // 计算所有按钮的文字宽度
    NSMutableArray *titleWidthArr = [[NSMutableArray alloc] init];
    [self.titleArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [titleWidthArr addObject:@(tempWidth)];
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            ...
            if (self.configure.isShowBadge) {
                CGFloat titleW =[titleWidthArr[index] floatValue];
                UILabel *badgeValue = [[UILabel alloc] init];
                badgeValue.frame = CGRectMake(btnW*0.5+titleW*0.5,kWidth_fact(5), 0, 0);
                badgeValue.textColor = [UIColor whiteColor];
                badgeValue.font = KF_H_F(6);
//                badgeValue.text = @"5";
                badgeValue.textAlignment = NSTextAlignmentCenter;
                KRadius(badgeValue, 5);
                badgeValue.backgroundColor = kColor_hex(@"#FF3B30");
                [btn addSubview:badgeValue];
                badgeValue.hidden = YES;
                [self.badgeArray addObject:badgeValue];
            }
...
/// 设置角标
/// @param index 下标  默认为 0
/// @param value 角标值  值 为 0 不显示
-(void)setBadge:(NSInteger)index value:(NSInteger)value{
    NSLog(@"%@",self.badgeArray);
    if (index<self.badgeArray.count) {
        UILabel *badgeValue = self.badgeArray[index];
        badgeValue.text = [NSString stringWithFormat:@"%ld",(long)value];
        if (badgeValue.text.length == 0 || value <= 0) {
            badgeValue.hidden = YES;
        }else{
            badgeValue.hidden = NO;
            CGFloat numWidth = kStr_width(badgeValue.text, kWidth_fact(100), KF_H_F(6));
            if (numWidth<10) {
                numWidth = 10;
            }
            badgeValue.frame = CGRectMake(badgeValue.left,badgeValue.top, numWidth+2, 10);
        }
    }
}

/// 清空角标
/// @param index 下标  - 1 为清空所有角标
-(void)clearBadge:(NSInteger)index{
    if (index == -1) {
        for (int i = 0; i< self.badgeArray.count; i++) {
            UILabel *badgeValue = self.badgeArray[i];
            badgeValue.text = @"0";
            badgeValue.hidden = YES;
        }
    }else{
