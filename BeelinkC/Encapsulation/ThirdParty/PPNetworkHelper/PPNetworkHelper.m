...
+ (void)initialize {
    ...
    _sessionManager = [AFAppDotNetAPIClient sharedClient];//[AFHTTPSessionManager manager];
    /*token 设置
    ...
    NSDictionary *dict = @{ NSHTTPCookieName : @"loginSessionId",
                            NSHTTPCookieValue : @"token",//[NSString memberLoginSessionId],
                            NSHTTPCookieDomain : @"",
                            NSHTTPCookiePath : @"/",
    ...
    */
    ...
    ///*
//     if ([[DDUserInfo share]isLogin]) {
//     [_sessionManager.requestSerializer setValue:[[DDUserInfo share] getToken] forHTTPHeaderField:@"authorization_token"];
//     [_sessionManager.requestSerializer setValue:[[DDUserInfo share] getKey] forHTTPHeaderField:@"authorization_key"];
//         
//         NSLog(@"token:%@<<=>>%@",[[DDUserInfo share] getToken],[[DDUserInfo share] getKey]);
//
//     }
    // */
    _sessionManager.requestSerializer.timeoutInterval = 20.f;
    _sessionManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/html", @"text/json", @"text/plain; charset=utf-8",@"text/plain", @"text/javascript", @"text/xml", @"image/*", nil];
    // 打开状态栏的等待菊花
