...
- (void)setMaxNumColor:(UIColor *)maxNumColor{
    ...
    self.numLabel.textColor = maxNumColor;
}

- (void)setMaxNumFont:(UIFont *)maxNumFont{
    _maxNumFont = maxNumFont;
    self.numLabel.font = maxNumFont;
}
...
//    }
    ...
    if ([textView.text length]>self.textMaxNum&&self.textMaxNum!=0&&textView.markedTextRange == nil) {
        textView.text = [textView.text substringToIndex:self.textMaxNum];
    }
    
    if (self.maxNumState == XMMaxNumStateNormal)
    {
        self.numLabel.text = [NSString stringWithFormat:@"%d/%d",(unsigned long)self.textView.text.length,self.textMaxNum];
    }else{
